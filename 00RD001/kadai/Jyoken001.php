...
<html>
<head><title>*** Jyoken001 ***</title></head>
<body>
<h1>条件指定検索プログラム１</h1>
<h2>条件指定による検索</h2>
<form method="post" action="Jyoken001.php">
<table border="1">
...
if(isset($_POST['submit'])){
    ...
    if(!empty($_POST['baisuu']))
        $sql.="* ".$_POST['baisuu'];
    $sql.=" as 与信限度額 FROM customer";
    
    if(isset($_POST['jyoken'])){
        $sql=Jyoken($sql);
    }
    ...
    switch($_POST['jyoken']){
            ...
            case "morethan":
                if(!empty($_POST['morethan1']))
                    $sql.=" WHERE C_Yoshin >".$_POST['morethan1'];
                break;
            case "lessthan":
                if(!empty($_POST['lessthan1']))
                    $sql.=" WHERE C_Yoshin <".$_POST['lessthan1'];
                break;
            case "morethan_lessthan":
                if(!empty($_POST['morethan_lessthan1'])&&!empty($_POST['morethan_lessthan2'])&&$_POST['morethan_lessthan2']>$_POST['morethan_lessthan1']){
                    ...
                    $sql.=" WHERE C_Yoshin BETWEEN ".$more." AND ".$less;
                }
                break;
            case "ijou":
                if(!empty($_POST['ijou1']))
                    $sql.=" WHERE C_Yoshin >=".$_POST['ijou1'];
                break; 
            case "ika":
                if(!empty($_POST['ika1']))
                    $sql.=" WHERE C_Yoshin <=".$_POST['ika1'];
                break; 
            case "ijou_ika":
                if(!empty($_POST['ijou_ika1'])&&!empty($_POST['ijou_ika2'])&&$_POST['ijou_ika2']>$_POST['ijou_ika1'])
                    $sql.=" WHERE C_Yoshin BETWEEN ".$_POST['ijou_ika1']." AND ".$_POST['ijou_ika2'];
                break;  
            default:
                break;
        }
